...
import notifications.resolvers  # pylint: disable=unused-import  # noqa: F401 — registers resolvers on import
...
class Application:
    ...
    async def process_report_real(
        ...
        file_path = await self.wb_client.download_documents(
            unique_id, count, chat_id, status_message_id
        )
            ...
            try:
                ...
                try:
                    task: ReportTask = await asyncio.wait_for(
                        self.report_queue.get_task(), timeout=10.0
                    )
            ...
            try:
                ...
                try:
                    result: ReportResult = await asyncio.wait_for(
                        self.report_queue.get_result(), timeout=10.0
                    )
                except asyncio.TimeoutError:
                    continue
                ...
                try:
                    ...
                    if result.success:
                        ...
                        if result.report_id:
                            await update_report_state(
                                result.report_id, ReportState.GENERATED
                            )
                            logger.info(
                                f"📝 Report {result.report_id} marked as GENERATED"
                            )
                        ...
                        if not os.path.exists(result.file_path):
                            raise FileNotFoundError(
                                f"File not found: {result.file_path}"
                            )
                        ...
                        balance_text = f"💰 <b>Ваш баланс:</b> {current_balance} отчетов\n\nВыберите действие ниже 👇"

                        keyboard = InlineKeyboardMarkup(
                            inline_keyboard=[
                                [
                                    InlineKeyboardButton(
                                        text="🔍 Сравнение карточек",
                                        callback_data="compare_cards",
                                    )
                                ],
                                [
                                    InlineKeyboardButton(
                                    ...
                                    InlineKeyboardButton(
                                        text="💬 Поддержка", url="https://t.me/wifftees"
                                    ),
                                ],
                                [
                                    InlineKeyboardButton(
                    ...
                    logger.error(
                        f"❌ Error processing result {result.task_id}: {e}",
                        exc_info=True,
                    )
    ...
    async def periodic_state_saver(self):
        ...
        """
        logger.info(
            f"💾 Periodic state saver started (interval: {settings.wb_state_save_interval}s)"
        )
    ...
    async def periodic_browser_restart(self):
        ...
        logger.info(
            f"🔄 Periodic browser restart started (interval: {interval}s / {interval // 3600}h)"
        )
            ...
            try:
                ...
                if self.wb_client:
                    ...
                    if self._active_reports_count == 0:
                        logger.info(
                            "✅ All reports completed, proceeding with browser restart"
                        )
                    ...
                    logger.warning(
                        "⚠️ WB client not initialized, skipping browser restart"
                    )
                ...
                logger.error(
                    f"❌ Error in periodic browser restart: {e}", exc_info=True
                )
    ...
    async def notification_worker(self):
        ...
        while not self._shutdown:
            try:
                ...
                logger.error(
                    f"❌ Error in notification worker cycle: {e}", exc_info=True
                )
    ...
    async def start(self):
        ...
        try:
            self.webhook_runner = await start_webhook_server(
                bot=self.bot, host=settings.webhook_host, port=settings.webhook_port
            )
            logger.info(
