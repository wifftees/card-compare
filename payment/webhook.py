...
async def handle_yookassa_webhook(
    data: Dict[str, Any], payment_service: PaymentService
) -> Dict[str, str]:
    """
    ...
    try:
        ...
        if event == "payment.succeeded":
            ...
            logger.info(
                f"🔑 [WEBHOOK] Processing payment.succeeded: order_id={order_id}, user_id={user_id}"
            )

            success = await payment_service.complete_payment(order_id)
            await create_event(
                CreateEventDTO(user_id=user_id, event_type=EventType.PAY_FOR_OPTION)
            )

            if success:
                logger.info(
                    f"✅ [WEBHOOK] Payment completed successfully: order_id={order_id}"
                )
                ...
                try:
                    ...
                    logger.error(
                        f"❌ [WEBHOOK] Failed to process referral bonus: {ref_err}",
                        exc_info=True,
                    )
            else:
                logger.error(
                    f"❌ [WEBHOOK] Failed to complete payment: order_id={order_id}"
                )

        elif event == "payment.canceled":
            if not order_id:
                logger.error(
                    "❌ [WEBHOOK] Missing order_id in metadata for canceled event"
                )
                return {"status": "ok", "error": "missing_order_id"}

            logger.info(
                f"🔑 [WEBHOOK] Processing payment.canceled: order_id={order_id}"
            )
            ...
            if success:
                logger.info(
                    f"✅ [WEBHOOK] Payment canceled successfully: order_id={order_id}"
                )
            else:
                logger.error(
                    f"❌ [WEBHOOK] Failed to cancel payment: order_id={order_id}"
                )
        ...
        logger.error(
            f"❌ [WEBHOOK] Unexpected error processing webhook: {e}", exc_info=True
        )
