...
class YookassaClient:
    ...
    def __init__(self, shop_id: str = None, secret_key: str = None):
        ...
        if not self.shop_id or not self.secret_key:
            ...
            masked_key = (
                self.secret_key[:8] + "..." if len(self.secret_key) > 8 else "***"
            )
            logger.info(
                f"🔑 YooKassa client initialized: shop_id={self.shop_id}, key={masked_key}"
            )
    ...
    async def create_payment(
        self, amount: float, order_id: str, description: str, user_id: int
    ) -> Dict[str, Any]:
        """
        ...
        expires_at = (datetime.utcnow() + timedelta(hours=12)).strftime(
            "%Y-%m-%dT%H:%M:%S.%f"
        )[:-3] + "Z"
        ...
        payload = {
            ...
            "receipt": {
                "customer": {
                    "email": "iliybugriy@gmail.com"  # Note: replace with real user email if available
                },
                "items": [
                    {
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        raise RuntimeError(f"Unexpected payment status: {status}")
            ...
            raise ConnectionError(f"Failed to connect to YooKassa: {e}") from e
