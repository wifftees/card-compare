...
async def _show_compare_cards_prompt(
...
async def request_compare_cards_callback(
    ...
    await create_event(
        CreateEventDTO(user_id=user.id, event_type=EventType.CLICK_COMPARE)
    )
...
async def cancel_compare_callback(callback: CallbackQuery, state: FSMContext):
    ...
    await create_event(
        CreateEventDTO(user_id=user_id, event_type=EventType.CLICK_CANCEL_COMPARE)
    )
    ...
    logger.info(
        f"✅ [COMPARE] Compare process cancelled and state cleared for user {user_id}"
    )
...
async def show_example_report_callback(callback: CallbackQuery, user: User):
    ...
    await create_event(
        CreateEventDTO(user_id=user.id, event_type=EventType.CLICK_EXAMPLE_REPORT)
    )
...
async def process_articles(
    message: Message, user: User, report_queue: ReportQueue, state: FSMContext
):
    ...
    await create_event(
        CreateEventDTO(user_id=user.id, event_type=EventType.ENTER_ARTICLES)
    )
    ...
    logger.info(
        f"Created compare task {task.task_id} with {len(articles)} articles, report_id={report_id}"
    )
